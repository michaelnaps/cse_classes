...
int main()
{
	...
	ofstream fout;  // file to be written to
	...
	if (rect_v.size() != 0) {
		...
		// print out how many rectangles are in the final vector to output file
		fout << "You have " << rect_v.size() << " rectangle(s) in you list." << endl;

		// for all elements of the vector
		for (int i(0); i < rect_v.size(); ++i) {
			// print the vector components to the output file
			rect_v[i].display(fout);
		}
	}
...
void Rectangle::display(ofstream& fout)
{
	// calculate midpoint
